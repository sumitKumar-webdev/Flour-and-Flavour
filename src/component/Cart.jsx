...
//  }
    ...
    <div className="bg-rose-50 min-h-screen py-8 px-4">
           ...
           <div className="grid lg:grid-cols-3 gap-6">
             <div className="lg:col-span-2 space-y-6">
                 ...
                 <div className="flex flex-col flex-1">
                   <h3 className="text-xl font-semibold text-gray-800">{ productDetail?.productName || 'Chocolate Truffle Cake'}</h3>
                   <p className="text-rose-600 text-lg font-semibold mt-1">₹ {(item?.price)?.toFixed(2) || 549}</p>
                   <p className="text-gray-500 text-sm">Weight: {item?.size}</p>
                   <div className="flex products-center gap-3 mt-3">
                     ...
                     <div className="flex item-center border caret-transparent border-rose-400 rounded-lg px-4  py-2 text-rose-600">
                       <span className="cursor-pointer font-bold hover:scale-105" onClick={()=>dispatch(decQty(item))}>-</span>
     ...
     {/* ADD Message Feature */}
                   <div className="text-sm text-gray-600 mt-2 gap-2 flex items-center ">
                     ...
                     {!showInput[passkey] && cakeMessage[passkey] && (<div className='font-semibold'>{cakeMessage[passkey]}</div>)}
                     
                     {!showInput[passkey] ? <span className="text-rose-500 font-medium cursor-pointer" 
                     onClick={()=>setShowInput(prev => ({
                      ...prev, 
                      [passkey]:true
                     }))}>

                     {cakeMessage[passkey] === undefined ? '+ Add' : '✏️ Edit'}
                     ...
                     onChange={(e)=>(setCakeMessage((prev)=>({ ...prev ,[passkey] : e.target.value})))}
                     />
                     <button 
                     onClick={()=>{
                      setShowInput((prev)=>({
                     ...
                     className='bg-rose-500 px-4 py-[6px] rounded-lg'
                     >Done
                     </button>
                     </div>
                     }
                 ...
                 //Skeleton loader
                 (
                  [1,2].map((index)=><div key={index} className="w-full flex gap-4 p-4 animate-pulse">
             ...
             <div className="bg-white max-h-[350px] p-5 rounded-2xl shadow-md">
               ...
               <div className="mt-5">
                 ...
                 className="w-full bg-rose-600 hover:bg-rose-700 text-white py-2 rounded-xl text-center font-semibold cursor-pointer">
                   <ShoppingCart className="inline mr-2" />Place Order
                 </button>
               </div>
  ...
  <div className="bg-white shadow-xl rounded-3xl p-16 flex flex-col items-center max-w-md w-full border border-rose-200">
    ...
    <p className="font-syne  text-gray-500 mt-2 text-center max-w-xs">
      Looks like you haven’t added anything yet. Find something sweet to treat yourself!
    </p>

    <button
      onClick={() => navigate('/')}
      className="mt-6 px-7 py-3 bg-rose-500 text-white rounded-xl hover:bg-rose-600 transition-colors duration-200"
    >
